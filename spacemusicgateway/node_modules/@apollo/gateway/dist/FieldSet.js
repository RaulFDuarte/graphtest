...
exports.groupByParentTypeAndResponseName = groupBy(field => `${field.scope.parentType}:${graphql_2.getResponseName(field.fieldNode)}`);
exports.groupByParentType = groupBy(field => field.scope.parentType);
function selectionSetFromFieldSet(fields, parentType) {
    return {
        kind: graphql_1.Kind.SELECTION_SET,
        selections: Array.from(exports.groupByParentType(fields)).flatMap(([typeCondition, fieldsByParentType]) => wrapInInlineFragmentIfNeeded(Array.from(exports.groupByParentTypeAndResponseName(fieldsByParentType).values()).map(fieldsByResponseName => {
